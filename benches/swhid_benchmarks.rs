...
fn bench_swhid_computation(c: &mut Criterion) {
    ...
    group.bench_function("content_swhid", |b| {
        b.iter(|| content.swhid())
    });
...
fn bench_qualified_swhid(c: &mut Criterion) {
    ...
    let core: Swhid = "swh:1:cnt:e69de29bb2d1d6434b8b29ae775ad8c2e48c5391".parse().unwrap();
...
fn bench_large_data(c: &mut Criterion) {
    ...
    group.bench_function("large_content_swhid", |b| {
        b.iter(|| content.swhid())
    });
